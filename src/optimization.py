...
def get_loss():
    ...
    # Cross Entropy Loss is the appropriate loss for multiclass
    # classification tasks such as this one (it combines LogSoftmax and
    # NLLLoss, which is why our models' forward() methods must NOT apply
    # softmax themselves).
...
def get_optimizer(
    ...
    if optimizer.lower() == "sgd":
        # Create an instance of the SGD optimizer using the input
        # parameters learning_rate, momentum and weight_decay
        opt = torch.optim.SGD(
            model.parameters(),
            lr=learning_rate,
            momentum=momentum,
        ...
        # Create an instance of the Adam optimizer using the input
        # parameters learning_rate and weight_decay (Adam does not use
        # a "momentum" parameter directly - it has its own beta terms)
