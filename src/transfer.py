...
def get_model_transfer_learning(model_name="resnet18", n_classes=50):
    ...
    # Freeze all parameters in the model. This way, during training, only
    # the newly-added final layer will be updated; the pretrained backbone
    # keeps the features it has already learned.
    ...
    #    outputs. Since it is newly created, its requires_grad is True by
    #    default, so it will be the only part of the network being trained.
